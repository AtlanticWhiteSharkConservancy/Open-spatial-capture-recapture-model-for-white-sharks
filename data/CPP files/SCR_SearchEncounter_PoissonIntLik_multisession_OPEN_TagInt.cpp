   ...
   for( int g=0; g<ngrid; g++){
    ...
    //   probcap(g,j,t) = p0 * exp( -alpha1*dist(g,j)*dist(g,j) ); // keep these separate to make clear
      //  probcap(g,j,t) = Type(1) - exp( -exp( alpha0(0) + alpha2*log_effort(j,t) ) * exp( -alpha1(0)*dist(g,j)*dist(g,j) ) ); 
        logpois(g,j,t) = alpha0( year(t) ) + alpha2*log_effort(j,t) - alpha1(0)*dist(g,j)*dist(g,j);
...
tpm(0,2,0) = 0;
tpm(1,0,0) = 0;
tpm(1,1,0) = phi(0); //if by month (1) because first t is July; if time specific or constant (0);
tpm(1,2,0) = 1 - phi(0);
...
//for( int g=0; g<ngrid; g++){
 //         // Megan getting wild
 //           for( int j=0; j<ntrap; j++ ){ 
 //             logpois(g,j,t) = alpha0( year(t) ) + alpha2*log_effort(j,t) - alpha1( taghist(i,t) )*dist(g,j)*dist(g,j);
 //             probcap(g,j,t) = Type(1) - exp( -exp( logpois(g,j,t) ) );
      //      }
   ...
   //     }
    ...
    // for first time step, tpm has already been applied via use of initial distribution
      ...
      // if tagged, use known state to inform encounter prob
      if( obs_state(i,t) == 1 & y.col(t).matrix().row(i).sum() == 0 ){ // if previously tagged and present, encountered with marginal prob
         enc_lik(0) = 0; // not encountered with 100% prob if not recruited
         enc_lik(1) = pow( margzenc, Kmax(t) ); //pow( marg_lik_zero(t), Kmax(t) ); // prob present but not seen
         enc_lik(2) = 0; // not encountered with 100% prob if dead
      // if not tagged
      } else {
       // if not seen
     ...
     for (int j=0; j<nstate; j++) { // j is current state
      ...
      if( obs_state(i,t) == 1 & y.col(t).matrix().row(i).sum() == 0 ){ // if previously tagged and present, encountered with marginal prob
        ...
        if( obs_state(i,t) == 0 ){ // if previously tagged and present, encountered with marginal prob
            enc_lik(0) = 0; // not encountered with 100% prob if not recruited
            enc_lik(1) = 0; // away so probability of being encoutered is 0
            enc_lik(2) = 1; // away for sure
        // if not tagged
       } else {
        if( y.col(t).matrix().row(i).sum() == 0 ){
         enc_lik(0) = 1; // not encountered with 100% prob if not recruited
