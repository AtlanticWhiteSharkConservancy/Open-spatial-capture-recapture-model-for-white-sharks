#########################################################################################
#### Support functions to create hexagonal or gridded, clipped polygons
...
##Establish conversion function##
km2_to_m2 <- function(from_km2){
  
  area_m2 <- from_km2 * 1000 * 1000 #Gets us m2
  return(area_m2)
  
}

## Modified function to extract grid cell centers
grid_coords <- function(x, type, cell_width, cell_area, clip = FALSE) {
  if (!type %in% c("square", "hexagonal")) {
    stop("Type must be either 'square' or 'hexagonal'")
  }
